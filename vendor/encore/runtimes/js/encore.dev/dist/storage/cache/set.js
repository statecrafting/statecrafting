 ...
 * @internal
 */
class SetKeyspace extends Keyspace {
     ...
     * @see https://redis.io/commands/spop/
     */
    async pop(key, count, options) {
        const source = getCurrentRequest();
        const mappedKey = this.mapKey(key);
        const ttlMs = this.resolveTtl(options);
        const results = await this.cluster.impl.spopN(mappedKey, count, ttlMs, source);
     ...
     */
    async diffStore(destination, ...keys) {
        if (keys.length === 0) {
            throw new Error("at least one key must be provided");
        }
        const source = getCurrentRequest();
     ...
     */
    async intersectStore(destination, ...keys) {
        if (keys.length === 0) {
            throw new Error("at least one key must be provided");
        }
        const source = getCurrentRequest();
     ...
     */
    async unionStore(destination, ...keys) {
        if (keys.length === 0) {
            throw new Error("at least one key must be provided");
        }
        const source = getCurrentRequest();
     ...
     * @see https://redis.io/commands/srandmember/
     */
    async sample(key, count) {
        if (count < 0) {
            throw new Error("count must be non-negative");
        }
        const source = getCurrentRequest();
        const mappedKey = this.mapKey(key);
        const results = await this.cluster.impl.srandmemberN(mappedKey, count, source);
        return results.map((r) => this.deserializeItem(r));
    }
    /**
     ...
     * @see https://redis.io/commands/srandmember/
     */
    async sampleWithReplacement(key, count) {
        if (count < 0) {
            throw new Error("count must be non-negative");
        }
        const source = getCurrentRequest();
     ...
     */
    async move(src, dst, member, options) {
        const source = getCurrentRequest();
 ...
 * ```
 */
export class StringSetKeyspace extends SetKeyspace {
    constructor(cluster, config) {
        super(cluster, config);
    }
    serializeItem(value) {
 ...
 * ```
 */
export class NumberSetKeyspace extends SetKeyspace {
    constructor(cluster, config) {
        super(cluster, config);
    }
    serializeItem(value) {
